...
/* scatter local poly */
graph tw (sca dpass vote if vote < 50) (sca dpass vote if vote >= 50), ytitle("Change in pass rate between years 0 and 2") xtitle("vote share") legend(off) xline(50) || lpolyci dpass vote if vote <50 & vote >=40, bwidth(1) lc(black) || lpolyci dpass vote if vote >=50 & vote <=60, bwidth(1) lc(black)
